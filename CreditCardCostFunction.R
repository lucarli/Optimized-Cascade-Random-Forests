#Cascade Random Forests cost function using a credit card dataset
...
anonymousCreditCard<-fread("Datasets\\CreditCartFraudDataset.csv", sep = ',', header = TRUE)

cost <- function(x){

  anonymousCreditCard$resp<-as.factor(anonymousCreditCard$Class)
  
  anonymousCreditCardN<-anonymousCreditCard[,c(-1,-31)]
  
  anonymousCreditCardNGenuine<-anonymousCreditCardN[anonymousCreditCardN$resp==0,]
  
  anonymousCreditCardNFraud<-anonymousCreditCardN[anonymousCreditCardN$resp==1,]
  
  mylist<-list()

  for(ind in 1:round(x[2])){

    subgenuine.idx<-sample(1:nrow(anonymousCreditCardNGenuine), nrow(anonymousCreditCardNFraud))
    
    subgenuine<-anonymousCreditCardNGenuine[subgenuine.idx,]
    
    subComplete<-rbind(subgenuine,anonymousCreditCardNFraud)
    ...
    train<-subComplete[train.idx,]
    
    test<-subComplete[-train.idx,]
